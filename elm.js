...
var $author$project$Game$initAsteroids = _List_fromArray(
	[
		{
		pos: {x: 50, y: 50},
		seed: 1.1,
...
var $author$project$Game$initGame = {
	asteroids: $author$project$Game$initAsteroids,
	bullets: _List_Nil,
	cooldown: 0,
	gameOver: false,
	score: 0,
...
var $author$project$Main$advanceTerminal = F5(
	function (timer, delta, lines, current, charIdx) {
		var typingSpeed = 35;
		var totalLines = $elm$core$List$length($author$project$Main$terminalContent);
		var newTimer = timer + delta;
		if (!totalLines) {
			return {charIdx: charIdx, current: current, lines: lines, timer: newTimer};
		} else {
			...
			if (_Utils_cmp(newTimer, typingSpeed) > -1) {
				...
				if (_v0.$ === 'Nothing') {
					...
					var targetLine = _v0.a;
					return (_Utils_cmp(
						charIdx,
						$elm$core$String$length(targetLine)) > -1) ? ((_Utils_cmp(newTimer, typingSpeed + 400) > -1) ? {
						charIdx: 0,
						current: '',
						lines: _Utils_ap(
							lines,
							_List_fromArray(
								[targetLine])),
						timer: 0
					} : {charIdx: charIdx, current: current, lines: lines, timer: newTimer}) : {
						charIdx: charIdx + 1,
						current: A2($elm$core$String$left, charIdx + 1, targetLine),
						lines: lines,
						timer: 0
					};
				}
...
var $author$project$Main$setKey = F3(
	function (key, val, keys) {
		switch (key) {
			case 'ArrowLeft':
...
var $author$project$Main$update = F2(
	function (msg, model) {
		switch (msg.$) {
			...
			case 'KeyDown':
				var key = msg.a;
				return _Utils_eq(model.focusedPanel, $author$project$Main$AsteroidsPanel) ? _Utils_Tuple2(
					_Utils_update(
						model,
						{
							keys: A3($author$project$Main$setKey, key, true, model.keys)
						}),
			...
			case 'ToggleCard':
				var idx = msg.a;
				var newSet = A2($elm$core$Set$member, idx, model.expandedCards) ? A2($elm$core$Set$remove, idx, model.expandedCards) : A2($elm$core$Set$insert, idx, model.expandedCards);
				return _Utils_Tuple2(
					_Utils_update(
						model,
		...
		[
			...
			_List_fromArray(
				[
					...
					A2($elm$html$Html$Attributes$style, 'width', '100%'),
					A2($elm$html$Html$Attributes$style, 'height', '100%'),
					A2($elm$html$Html$Attributes$style, 'background', 'radial-gradient(ellipse at center, transparent 50%, rgba(10,10,12,0.8) 100%)'),
					A2($elm$html$Html$Attributes$style, 'pointer-events', 'none'),
...
var $author$project$Main$viewFooter = A2(
	...
	_List_fromArray(
		[
			...
			A2($elm$html$Html$Attributes$style, 'padding', '0 24px 80px'),
		...
		[
			...
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							$elm$html$Html$text('rendered in elm')
						]))
				]))
		]));
var $author$project$Main$viewGridFloor = A2(
	$elm$html$Html$div,
	_List_fromArray(
		[
			...
			A2($elm$html$Html$Attributes$style, 'mask-image', 'linear-gradient(to top, rgba(0,0,0,0.3) 0%, transparent 70%)'),
...
var $author$project$Main$viewHero = A2(
	...
	_List_fromArray(
		[
			$author$project$Main$viewGridFloor,
			A2(
			$elm$html$Html$div,
			_List_fromArray(
				[
...
var $author$project$Cards$Education = {$: 'Education'};
var $author$project$Cards$Experience = {$: 'Experience'};
var $author$project$Cards$Project = {$: 'Project'};
var $author$project$Cards$crystalData = _List_fromArray(
	[
		{
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							$elm$html$Html$Attributes$class('card-scan'),
							A2($elm$html$Html$Attributes$style, 'position', 'absolute'),
							A2($elm$html$Html$Attributes$style, 'top', '-4px'),
							A2($elm$html$Html$Attributes$style, 'left', '0'),
							A2($elm$html$Html$Attributes$style, 'width', '100%'),
...
var $author$project$Sand$viewSandPanelInner = function (_v0) {
	return A2(
		...
		_List_fromArray(
			[
				A2($elm$html$Html$Attributes$style, 'animation', 'fadeUp 0.6s ease 0.2s both')
			]),
		_List_fromArray(
			[
				A2(
				...
				_List_fromArray(
					[
						$elm$html$Html$Attributes$id('sand-container'),
						A2($elm$html$Html$Attributes$style, 'position', 'relative'),
						A2($elm$html$Html$Attributes$style, 'width', '100%'),
				...
				_List_fromArray(
					[
						$elm$html$Html$text('R to reset')
					]))
			]));
};
var $author$project$Sand$viewSandPanel = function (state) {
	return A2($elm$html$Html$Lazy$lazy, $author$project$Sand$viewSandPanelInner, state);
};
var $author$project$Main$viewSectionDivider = function (label) {
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
			[
...
var $author$project$Main$viewTerminalActiveLine = function (current) {
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
			[
...
var $author$project$Main$viewTerminal = function (model) {
	...
	var currentTyping = model.terminalCurrentLine;
	var allLines = model.terminalLines;
	var visibleLines = A2(
		$elm$core$List$drop,
		A2(
			$elm$core$Basics$max,
			0,
			$elm$core$List$length(allLines) - maxVisibleLines),
		allLines);
	return A2(
		$elm$html$Html$div,
		_List_Nil,
		_List_fromArray(
			[
				...
				_Utils_ap(
					A2($elm$core$List$map, $author$project$Main$viewTerminalLine, visibleLines),
					_List_fromArray(
						[
							$author$project$Main$viewTerminalActiveLine(currentTyping)
						])))
			]));
};
	...
	return A2(
		...
		_List_fromArray(
			[
				A2($elm$html$Html$Attributes$style, 'max-width', '1200px'),
				A2($elm$html$Html$Attributes$style, 'margin', '0 auto'),
				A2($elm$html$Html$Attributes$style, 'padding', '80px 24px 120px'),
				A2($elm$html$Html$Attributes$style, 'position', 'relative'),
				A2($elm$html$Html$Attributes$style, 'z-index', '1')
			]),
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$text('$ cat /data/fragments.log')
							])),
						A2(
						$elm$html$Html$div,
						_List_fromArray(
								...
								_List_fromArray(
									[
										...
										$elm$core$String$fromInt(
											$elm$core$List$length($author$project$Cards$crystalData)) + ' entries')
									]))
							]))
					])),
				$author$project$Main$viewSectionDivider('SANDBOX'),
				A2(
				$elm$html$Html$div,
				_List_fromArray(
					[
				...
				_List_fromArray(
					[
						$author$project$Sand$viewSandPanel(model.sand)
					])),
				$author$project$Main$viewSectionDivider('EXPERIENCE'),
				A2(
				$elm$html$Html$div,
				_List_fromArray(
					[
...
var $author$project$Main$viewMarquee = A2(
	$elm$html$Html$div,
	_List_fromArray(
		[
			A2($elm$html$Html$Attributes$style, 'position', 'relative'),
	...
	_List_fromArray(
		[
			...
			_List_fromArray(
				[
					$elm$html$Html$text('python  //  sql  //  rust  //  java  //  scala  //  golang  //  c++  //  c  //  elixir  //  lua  //  bash  //  docker  //  neovim  //  git  //  unix  //  spark  //  datafusion  //  python  //  sql  //  rust  //  java')
				]))
		]));
var $author$project$Starfield$blackHoles = _List_fromArray(
	[
		{cycleSpeed: 18000, mass: 22, phase: 0, x: 18, y: 25},
				...
				[
					...
					_List_fromArray(
						[
							...
							$elm$html$Html$Attributes$style,
							'width',
							$elm$core$String$fromFloat(diskSize * 4) + 'px'),
							A2(
							$elm$html$Html$Attributes$style,
							'height',
							$elm$core$String$fromFloat(diskSize * 1.2) + 'px'),
							A2($elm$html$Html$Attributes$style, 'border-radius', '50%'),
							A2(
							$elm$html$Html$Attributes$style,
							'border',
							...
							$elm$html$Html$Attributes$style,
							'opacity',
							$elm$core$String$fromFloat(coreVisibility))
						]),
					_List_Nil) : $elm$html$Html$text(''),
					(coreGlow > 0.01) ? A2(
					$elm$html$Html$div,
					_List_fromArray(
						[
							...
							$elm$html$Html$Attributes$style,
							'opacity',
							$elm$core$String$fromFloat(coreGlow))
						]),
					_List_Nil) : $elm$html$Html$text(''),
					(flashOpacity > 0.01) ? A2(
					$elm$html$Html$div,
					_List_fromArray(
						[
							A2($elm$html$Html$Attributes$style, 'position', 'absolute'),
							...
							$elm$html$Html$Attributes$style,
							'opacity',
							$elm$core$String$fromFloat(flashOpacity))
						]),
					_List_Nil) : $elm$html$Html$text('')
				]));
	});
...
var $author$project$Starfield$viewFerris = F2(
	function (time, ferris) {
		...
		var s = ferris.size;
		var finalRotation = ferris.rotation + spin;
		var driftY = $elm$core$Basics$sin((time / 18000) + (ferris.seed * 0.7)) * 4;
		var driftX = $elm$core$Basics$cos((time / 15000) + ferris.seed) * 3;
		return A2(
			$elm$html$Html$div,
			_List_fromArray(
				[
					...
					$elm$html$Html$Attributes$style,
					'left',
					'calc(' + ($elm$core$String$fromFloat(ferris.x + driftX) + ('% - ' + ($elm$core$String$fromFloat(s / 2) + 'px)')))),
					A2(
					$elm$html$Html$Attributes$style,
					'top',
					'calc(' + ($elm$core$String$fromFloat(ferris.y + driftY) + ('% - ' + ($elm$core$String$fromFloat(s / 2) + 'px)')))),
					A2(
					$elm$html$Html$Attributes$style,
					'width',
					$elm$core$String$fromFloat(s) + 'px'),
					A2(
					$elm$html$Html$Attributes$style,
					'height',
					$elm$core$String$fromFloat(s) + 'px'),
					...
					$elm$html$Html$Attributes$style,
					'opacity',
					$elm$core$String$fromFloat(ferris.opacity)),
					A2(
					$elm$html$Html$Attributes$style,
					'transform',
					'rotate(' + ($elm$core$String$fromFloat(finalRotation) + 'deg)'))
				]),
			_List_fromArray(
				[
					A2(
					...
					_List_fromArray(
						[
							$elm$svg$Svg$Attributes$viewBox('0 0 120 100'),
							$elm$svg$Svg$Attributes$width(
							$elm$core$String$fromFloat(s)),
							$elm$svg$Svg$Attributes$height(
							$elm$core$String$fromFloat((s * 100) / 120)),
						...
						[
							...
							_List_fromArray(
								[
									...
									$elm$svg$Svg$Attributes$cy('38'),
									$elm$svg$Svg$Attributes$rx('28'),
									$elm$svg$Svg$Attributes$ry('26'),
									$elm$svg$Svg$Attributes$fill('rgba(168,85,247,0.15)'),
							...
							_List_fromArray(
								[
									$elm$svg$Svg$Attributes$d('M42 28 Q52 18 68 22'),
									$elm$svg$Svg$Attributes$stroke('rgba(200,180,255,0.4)'),
							...
							_List_fromArray(
								[
									$elm$svg$Svg$Attributes$cx('60'),
									$elm$svg$Svg$Attributes$cy('58'),
									$elm$svg$Svg$Attributes$rx('32'),
									$elm$svg$Svg$Attributes$ry('20'),
									$elm$svg$Svg$Attributes$fill('rgba(220,80,50,0.85)'),
									$elm$svg$Svg$Attributes$stroke('rgba(220,80,50,0.5)'),
									$elm$svg$Svg$Attributes$strokeWidth('1')
								]),
							_List_Nil),
							A2(
							...
							_List_fromArray(
								[
									$elm$svg$Svg$Attributes$d('M44 50 Q52 42 60 50 Q68 42 76 50'),
									$elm$svg$Svg$Attributes$stroke('rgba(180,60,40,0.6)'),
							...
							_List_fromArray(
								[
									...
									$elm$svg$Svg$Attributes$fill('rgba(240,240,255,0.9)')
								]),
							_List_Nil),
							A2(
							$elm$svg$Svg$circle,
							...
							_List_fromArray(
								[
									$elm$svg$Svg$Attributes$cx('51'),
									$elm$svg$Svg$Attributes$cy('40'),
									$elm$svg$Svg$Attributes$r('1.5'),
									$elm$svg$Svg$Attributes$fill('rgba(20,20,30,0.9)')
								]),
							_List_Nil),
							A2(
							$elm$svg$Svg$circle,
							...
							_List_fromArray(
								[
									$elm$svg$Svg$Attributes$d('M28 58 L18 50 L14 56 L22 60 Z'),
									$elm$svg$Svg$Attributes$fill('rgba(220,80,50,0.75)'),
									$elm$svg$Svg$Attributes$stroke('rgba(220,80,50,0.5)'),
									$elm$svg$Svg$Attributes$strokeWidth('0.8')
								]),
							_List_Nil),
							...
							_List_fromArray(
								[
									$elm$svg$Svg$Attributes$d('M92 58 L102 50 L106 56 L98 60 Z'),
									$elm$svg$Svg$Attributes$fill('rgba(220,80,50,0.75)'),
									$elm$svg$Svg$Attributes$stroke('rgba(220,80,50,0.5)'),
									$elm$svg$Svg$Attributes$strokeWidth('0.8')
								]),
							...
							_List_fromArray(
								[
									$elm$svg$Svg$Attributes$d('M38 70 L28 82 M44 72 L36 86 M50 73 L44 88'),
									$elm$svg$Svg$Attributes$stroke('rgba(220,80,50,0.6)'),
							...
							_List_fromArray(
								[
									$elm$svg$Svg$Attributes$d('M82 70 L92 82 M76 72 L84 86 M70 73 L76 88'),
									$elm$svg$Svg$Attributes$stroke('rgba(220,80,50,0.6)'),
									$elm$svg$Svg$Attributes$strokeWidth('1.2'),
									$elm$svg$Svg$Attributes$strokeLinecap('round')
								]),
							...
							_List_fromArray(
								[
									$elm$svg$Svg$Attributes$cx('78'),
									$elm$svg$Svg$Attributes$cy('3'),
...
var $author$project$Starfield$viewStar = F2(
	function (time, star) {
		var twinkle = function () {
			var t3 = $elm$core$Basics$cos((time / 1900) + (star.seed * 1.91));
			var t2 = $elm$core$Basics$sin((time / 2800) + (star.seed * 7.13));
			var t1 = $elm$core$Basics$sin((time / 1200) + (star.seed * 3.7));
			return 0.75 + (((t1 * 0.1) + (t2 * 0.08)) + (t3 * 0.07));
		}();
		var timeDriftY = ($elm$core$Basics$sin((time / 20000) + star.seed) * 8) * star.depth;
		var timeDriftX = ($elm$core$Basics$cos((time / 25000) + (star.seed * 0.7)) * 4) * star.depth;
		var grav = A3(
			$elm$core$List$foldl,
		...
		return (finalOpacity < 0.01) ? $elm$html$Html$text('') : A2(
			...
			_List_fromArray(
				[
					...
					$elm$html$Html$Attributes$style,
					'width',
					$elm$core$String$fromFloat(finalSize) + 'px'),
					A2(
					$elm$html$Html$Attributes$style,
					'height',
					$elm$core$String$fromFloat(finalSize) + 'px'),
					A2($elm$html$Html$Attributes$style, 'border-radius', '50%'),
					A2($elm$html$Html$Attributes$style, 'background', 'rgba(220,215,240,0.9)'),
					A2(
					$elm$html$Html$Attributes$style,
					'opacity',
					$elm$core$String$fromFloat(finalOpacity)),
					A2(
					$elm$html$Html$Attributes$style,
					'box-shadow',
...
var $author$project$Main$view = function (model) {
	return {
		body: _List_fromArray(
			[
				$author$project$Main$injectStyles,
